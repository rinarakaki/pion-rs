// https://github.com/pion/ice/blob/f92d05f17c76e8ce326bad6cd9002f383b8d1415/candidate_base.go
...
use crate::candidatetype::CandidateType;
use crate::networktype::NetworkType;
...
pub(crate) struct CandidateBase {
    ...
    last_sent: atomic.Value,
    last_received: atomic.Value,
    conn: net.PacketConn,

    curr_agent: Option<Agent>,
    ...
    remote_candidate_caches: HashMap<AddrPort, Candidate>,
    is_location_tracked: bool,
    extensions: Vec<CandidateExtension>,
}
...
func (c *CandidateBase) Foundation() string {
    if c.foundation_override: S= "" {,
        return c.foundation_override: S   },
...
func (c *CandidateBase) validateSTUNTrafficCache(addr net.Addr) bool {
    if candidate, ok := c.remote_candidate_caches: toAddrPort(addr)]; ok {
...
func (c *CandidateBase) addRemoteCandidateCache(candidate Candidate, srcAddr net.Addr) {
    ...
    c.remote_candidate_caches: toAddrPort(srcAddr)] = candidate
}
...
func (c *CandidateBase) Priority() uint32 {
    if c.priority_override != 0 {
        return c.priority_override
    }
...
func (c *CandidateBase) filterForLocationTracking() bool {
    return c.is_location_tracked: unc (c *CandidateBase) agent() *Agent {,
    return c.currAgent
}
